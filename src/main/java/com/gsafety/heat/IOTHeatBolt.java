 ...
 * Written on 17/8/29.
 */
public class IOTHeatBolt extends BaseBasicBolt {
    ...
    private void pullDynamicConfig() {
        ...
        PathChildrenCache pathChildrenCache = new PathChildrenCache(client, SystemConfig.get("ZK_DYNAMIC_CONFIG_HEAT"), true);
        try {
                ...
                public void childEvent(CuratorFramework curatorFramework, PathChildrenCacheEvent event) throws Exception {
                    ...
                    switch (event.getType()) {
                        case CHILD_ADDED: {
                            ...
                            List<ThresholdValueCommon> heatList = getThresholdValueCommon(dataStr);
                            if (heatList != null && heatList.size() > 0) {
                                configMap.put(path, heatList);
                            }
                            break;
                        }
                        case CHILD_UPDATED: {
    ...
    private void handleAlarm(String sensorId, List<SensorDataEntry> sensorDataEntries, SensorData sensorData) {
        ...
        List<ThresholdValueCommon> heatAramList = configMap.get(sensorId);
        //检查配置如果配置了
        if (heatAramList != null && heatAramList.size() > 0) {
            for (SensorDataEntry sensorDataEntry : sensorDataEntries) {

                ThresholdValueCommon heatAramConfig = getRightConfig(heatAramList, sensorDataEntry);
                if (heatAramConfig == null) {
                    break;
                }
                Float value = sensorDataEntry.getValues().get(0);

                if (value < heatAramConfig.getAlarmFirstLevelUp() && value > heatAramConfig.getAlarmFirstLevelDown()) {
                    sensorDataEntry.setLevel(1);
                } else if ((value < heatAramConfig.getAlarmSecondLevelUp() && value > heatAramConfig.getAlarmSecondLevelDown())) {
                    sensorDataEntry.setLevel(2);
                } else if (value < heatAramConfig.getAlarmThirdLevelUp() && value > heatAramConfig.getAlarmThirdLevelDown()) {
    ...
    private void writeToAlarmTopic(String sensorId, SensorDataEntry sensorDataEntry, SensorData sensorData) {
        ...
        sensorDetail.setValues(values);
        //写到报警topic
        producer.send(new ProducerRecord<>(SystemConfig.get("HEAT_ALARM_TOPIC"), sensorId, AvroUtil.serializer(sensorDetail)));
    }
    ...
    public List<ThresholdValueCommon> getThresholdValueCommon(String json) {
        ...
        JSONObject jsonMid = JSONObject.parseObject(json);//key  heat_fixation value arrayList
